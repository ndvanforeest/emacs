...
Often I use the (very handy) \LaTeX\/ =subfiles=package. The next allows to export (=C-c C-o l l=) to a =\documentclass{subfile}=.
I want to be able to export to the a subfiles package
